...
req_url= 'http://portal-server/nagios/connectivityFirewallAPI.jsp?requestType=get&status=Approved%20by%20hosting'
...
def close_request(portal_id):
	try:
		url = ('http://portal.traiana.com/nagios/connectivityFirewallAPI.jsp?requestType=setStatus&id=%s&status=Done' % (portal_id))
		urllib2.urlopen(url)
	except:
...
if __name__ == '__main__':
    ...
    for req in json_req:
        ...
        if req['environment'] == 'UAT' and req['status'] == 'Approved by hosting':
            ...
            if (connect_method.prog) in req['connectionMethod'] == 'FIX (SSL Port 443)' and req['overRadianz'] == 'No':
                ...
                if "Incoming" in req['direction']:
                    print req[ 'environment' ] + " is " + req['status'] + " and need to open " + connect_method.prog + " Connecionn Method is " + req[ 'direction' ]
                    for client_IP in ''.join(req[ 'clientSourceIpAddress()' ]).split(','):
                        ...
                        set_config.append('set security address-book global address-set %s address %s' % (policy_group_name, client_name))
                    else:
                        portal_id = req['id']
                    ...
                    for Client_Port in ''.join(req['Other (specify port)' ]).split(','):
                        ...
                        set_config.append('set  applications  application  %s  protocol  tcp destination-port %s' % (TCP_port_name, SingleClient_Port))
                        set_config.append('set  applications  application-set %s application %s' % (policy_group_name, TCP_port_name))
                    else:
                        portal_id = req[ 'id' ]
                        connect('192.168.160.1', 'root', password, set_config, portal_id)
            # FIX / Other (specify port) -  Outgoing to CME only
            elif (connect_method.prog) in req['connectionMethod'] == 'Other (specify port)' and req['clientName'] == 'CME' and req['overRadianz'] == 'No':
            ...
            elif (connect_method.prog) in req[ 'connectionMethod' ] == 'Other (specify port)' and (bloomberg_parse.prog) in req['clientName'] !='Bloomberg' and req['overRadianz' ] == 'No':
                if "Outgoing" in req['direction']:
                    set_config[ : ] = []
                    for Client_Port in ''.join(req[ 'mqAndOtherPort' ]).split(','):
                        SingleClient_Port = Client_Port.strip()
            ...
            elif (connect_method.prog) in req['connectionMethod'] == 'FIX (NON SSL Port 443)' and req[ 'overRadianz' ] == 'No':
                if "Incoming" in req['direction']:
                    set_config[:] = []
                    for client_IP in ''.join(req[ 'clientSourceIpAddress()']).split(','):
                        single_IP = client_IP.strip()
            ...
            # FTP
                ...
                if "Incoming" in req['direction']:
                    ...
                    for client_IP in ''.join(req['clientSourceIpAddress()']).split(','):
                        ...
                        connect('192.168.160.1', 'root', password, set_config, portal_id)
            # TOF (TCP port 5003)
            elif (connect_method.prog) in req[ 'connectionMethod' ] == 'TOF (TCP port 5003)' and req[ 'overRadianz' ] == 'No':
                if "Incoming" in req[ 'direction' ]:
                    set_config[:] = []
                    for client_IP in ''.join(req[ 'clientSourceIpAddress()' ]).split(','):
                        single_IP = client_IP.strip()
            ...
            elif (connect_method.prog) in req[ 'connectionMethod' ] == 'FIX LimitHub (SSL Port 443)'or req['connectionMethod' ] == 'FIX LimitHub (NON-SSL Port 3400 - Radianz)' and req[ 'overRadianz' ] == 'No':
                if "Incoming" in req[ 'direction']:
                    set_config[:] = []
                    for client_IP in ''.join(req[ 'clientSourceIpAddress()' ]).split(','):
                        single_IP = client_IP.strip()
                        ...
                        set_config.append('set security address-book global address %s %s' % (client_name, single_IP))
                        set_config.append('set security address-book global address-set %s address %s' % (policy_group_LHub_In, client_name))
                    else:
                        portal_id = req[ 'id' ]
                        connect('192.168.160.1', 'root', password, set_config, portal_id)
                    ...
                    for client_IP in ''.join(req['clientSourceIpAddress()']).split(','):
                        ...
                        set_config.append('set security address-book global address %s %s' % (client_name, single_IP))
                        set_config.append('set security address-book global address-set %s address %s' % (policy_group_LHub_In, client_name))
                        set_config.append('set security address-book global address-set %s address %s' % (policy_group_LHub_Out, client_name))
                    else:
                        portal_id = req[ 'id' ]
                        connect('192.168.160.1', 'root', password, set_config, portal_id)
            ...
            elif (connect_method.prog) in req['connectionMethod' ] == 'MQ (specify port)' and req['overRadianz'] == 'Yes':
                if "Outgoing" in req[ 'direction' ]:
                    set_config[:] = []
                    for Client_Port in ''.join(req['mqAndOtherPort']).split(','):
                        SingleClient_Port = Client_Port.strip()
            ...
            # Over  Radianz- MQ(port 1414/1421/5415)
                elif 'Incoming' in req['direction'] and '1414' in req['mqAndOtherPort'] or '1421' in req['mqAndOtherPort'] or '5415' in req['mqAndOtherPort']:
                    set_config[:] = []
                    for client_IP in ''.join(req[ 'clientSourceIpAddress()' ]).split(','):
                        single_IP = client_IP.strip()
                        client_name = '_'.join(req[ 'clientName' ].split()).upper() + '_' + ''.join(single_IP)
            ...
            elif (connect_method.prog) in req[ 'connectionMethod' ] == 'Other (specify port)' and req['overRadianz' ] == 'Yes':
                if "Outgoing" in req[ 'direction' ]:
                    set_config[ : ] = [ ]
                    for Client_Port in ''.join(req[ 'mqAndOtherPort' ]).split(','):
                        SingleClient_Port = Client_Port.strip()
            ...
            elif (connect_method.prog) in req[ 'connectionMethod' ] == 'FIX LimitHub (SSL Port 443)' or req['connectionMethod' ] == 'FIX LimitHub (NON-SSL Port 3400 - Radianz)' and req['overRadianz' ] == 'Yes':
                if "Incoming" in req[ 'direction' ]:
                    set_config[ : ] = [ ]
                    for client_IP in ''.join(req[ 'clientSourceIpAddress()' ]).split(','):
                        single_IP = client_IP.strip()
            ...
            # FTP LHUB_From_EXTERNAL
                ...
                if "Incoming" in req['direction']:
                    ...
                    for client_IP in ''.join(req['clientSourceIpAddress()']).split(','):
                        ...
                        connect('192.168.160.1', 'root', password, set_config, portal_id)
            # FIX (SSL Port 443) Incoming & Outgoing
            elif (connect_method.prog) in req[ 'connectionMethod' ] == 'FIX (SSL Port 443)' and req['overRadianz' ] == 'Yes':
                if "Incoming" in req[ 'direction' ]:
                    set_config[ : ] = [ ]
                    for client_IP in ''.join(req[ 'clientSourceIpAddress()' ]).split(','):
                        single_IP = client_IP.strip()
            ...
            elif (connect_method.prog) in req[ 'connectionMethod' ] == 'TOF (TCP port 5003)' and req[ 'overRadianz' ] == 'Yes':
                if "Incoming" in req[ 'direction' ]:
                    set_config[:] = []
                    for client_IP in ''.join(req[ 'clientSourceIpAddress()' ]).split(','):
                        single_IP = client_IP.strip()
            ...
            # sFTP  Radianz
                ...
                print "Could not found any match check "
