...
def connect(host, def_username, password, set_config, portal_id, port='830'):
    ...
    try:
        ...
        for line in set_config:
            ...
            conn.unlock()
            conn.close_session()
            print 'Connectivity portal id:  %s has been update on FW and close successfully ' % (portal_id)
            close_request(portal_id)
            message = 'Connectivity portal id:  %s has been update on FW and close successfully ' % (portal_id)
            send_email(message)
...
###############################################################################################
        ...
        ###############################################################################################
        # LIMITHUB PROD Approved by hosting condition match
        ###############################################################################################
        if req['environment'] == 'LimitHub' and req['status'] == 'CONNECTIVITY_APPROVED_READY_FOR_CREATION':
            print req['environment'] + " is " + req['status']
            connect_method = argparse.ArgumentParser(req['connectionMethod'])

        ###############################################################################################
        # LIMITHUB PROD // FIX (SSL port 443) // FIX (NON-SSL port 3400 - Radianz)
        ###############################################################################################
        elif (connect_method.prog) in req['connectionMethod'] == 'FIX (SSL port 443)' or req[
            'connectionMethod'] == 'FIX (NON-SSL port 3400 - Radianz)' and req['overRadianz'] == 'No':
            if "Incoming" in req['direction']:
                set_config[:] = []
                for client_IP in ''.join(req['clientSourceIpAddress()']).split(','):
                    single_IP = client_IP.strip()
                    ...
                    portal_id = req['id']
                    connect('10.203.100.1', 'root', password, set_config, portal_id)

        ###############################################################################################
        # LIMITHUB PROD // MQ (specify port)
        ###############################################################################################
        elif (connect_method.prog) in req['connectionMethod'] == 'MQ (specify port)':
            if "Incoming" in req['direction']:
                set_config[:] = []
                for client_IP in ''.join(req['clientSourceIpAddress()']).split(','):
                    single_IP = client_IP.strip()
    ...
    #              send_email(message)
        ...
        print 'Could not find any portal id with status: Approved by hosting'
