...
def connect(host, def_username, password, set_config, portal_id, port='830'):
    ...
    try:
        ...
        for line in set_config:
            ...
            conn.unlock()
            conn.close_session()
            print 'Connectivity portal id:  %s has been update on FW LH and close successfully ' % (portal_id)
            close_request(portal_id)
            message = 'Connectivity portal id:  %s has been update on FW LH and close successfully ' % (portal_id)
            send_email(message)
...
if __name__ == '__main__':
    ...
    for req in json_req:
        ...
        portal_id = req['id']
        ##################################################
        # LimitHub and Approved by hosting condition match
        ##################################################
